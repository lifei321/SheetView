...
- (id)initWithTitle:(NSString *)title delegate:(id)delegate cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitles:(NSArray *)otherButtonTitles {
    if (self = [super initWithFrame:[UIScreen mainScreen].bounds]) {
        ...
        _buttonTitleArray = [NSMutableArray arrayWithArray:otherButtonTitles];
...
- (void)initContentView {
    ...
    if (contentViewHeight < 4*self.frame.size.height/5) {
        ...
        _contentView.frame = CGRectMake((self.frame.size.width - contentViewWidth ) / 2, self.frame.size.height, contentViewWidth, 4*self.frame.size.height/5);
    }
    
    _contentView.contentSize = CGSizeMake(0, contentViewHeight);
...
- (void)initButtons {
    if (_buttonTitleArray.count > 0) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            [button setTitleColor:[UIColor colorWithRed:248 / 255.0 green:125 / 255.0 blue:50 / 255.0 alpha:1.0] forState:UIControlStateNormal];
